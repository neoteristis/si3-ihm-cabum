...
public class Factory extends EventFactory {
    ...
    @Override
    public Event build(EventType type, String description, byte[] image, GeoPoint address, Date time) throws Throwable {
        return build(type, description, image, address, time, 0);
    }

    @Override
    public Event build(EventType type, String description, byte[] image, GeoPoint address, Date time, int numberOfApproval) throws Throwable {
